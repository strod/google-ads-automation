 ...
 *
 * Version: 3.3
 * Updated 2023-02-15 to fix bug with shopping campaigns
 * dev.thrive@umww.com
 *
 */
...
function main() {
  ...
  var spreadsheetUrl = 'https://docs.google.com/spreadsheets/d/1794Y4CjtH6ppZYPlugQxR3awFawcYCey9VPtaAyEfzg/edit#gid=1172651668';

  var bidMultiplier_sheet = "Bidding Multipliers"
  ...
  var selectedCampaignIds = ['17428857724']
  ...
  var sheet = spreadsheet.getSheetByName(bidMultiplier_sheet);
  ...
  for (var h = 0; h < 5; h++) {
    ...
    if (h < 4) {
      ...
      if (isNaN(bidModifier) || (bidModifier < -0.9 && bidModifier > -1) || bidModifier > 9) {
        ...
        Logger.log("Bid modifier '" + bidModifier + "' for " + weekDays[newDay] + ' ' + newHour);
      }
    } else {
  ...
  var campaignSelector = ConstructIterator(shoppingCampaigns).withIds(selectedCampaignIds);
  ...
  if (runMobileBids) {
    if (sheets.length < 2) {
      ...
      sheet.getRange('J4').setValue(totalMultiplier);
